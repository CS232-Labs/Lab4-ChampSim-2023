...
void champsim::plain_printer::print(O3_CPU::stats_type stats)
{
  ...
  fmt::print(stream, "\n{} cumulative IPC: {:.4g} instructions: {} cycles: {} base_update_uops: {}\n", stats.name, std::ceil(stats.instrs()) / std::ceil(stats.cycles()),
             stats.instrs(), stats.cycles(), stats.base_update_uops());
  ...
  stream << "Branch MPKI breakdown" << std::endl;
  stream << "  Direction_miss: " << 1000.0 * stats.direction_prediction_miss / (double) stats.instrs() << " " << stats.direction_prediction_miss << std::endl;
  stream << "  BTB_miss_gen_miss: " << 1000.0 * stats.target_prediction_miss_general_not_in_btb / (double) stats.instrs() << " " <<stats.target_prediction_miss_general_not_in_btb << std::endl;
  stream << "  BTB_miss_gen_wrong: " << 1000.0 * stats.target_prediction_miss_general_wrong_target / (double) stats.instrs() << " " <<stats.target_prediction_miss_general_wrong_target << std::endl;
  stream << "  BTB_miss_indirect_miss: " << 1000.0 * stats.target_prediction_miss_indirect_not_in_btb / (double) stats.instrs() << " " <<stats.target_prediction_miss_indirect_not_in_btb << std::endl;
  stream << "  BTB_miss_indirect_wrong: " << 1000.0 * stats.target_prediction_miss_indirect_wrong_target / (double) stats.instrs() << " " <<stats.target_prediction_miss_indirect_wrong_target << std::endl;
  stream << "  BTB_miss_return_miss: " << 1000.0 * stats.target_prediction_miss_return_not_in_btb / (double) stats.instrs() << " " <<stats.target_prediction_miss_return_not_in_btb << std::endl;
  stream << "  BTB_miss_return_wrong: " << 1000.0 * stats.target_prediction_miss_return_wrong_target / (double) stats.instrs() << " " <<stats.target_prediction_miss_return_wrong_target << std::endl;
  stream << std::endl;
}
