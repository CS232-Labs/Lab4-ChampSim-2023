...
void O3_CPU::begin_phase()
{
  ...
  stats.begin_base_update_uops = num_base_update_uops;
...
void O3_CPU::end_phase(unsigned finished_cpu)
{
  ...
  sim_stats.end_base_update_uops = num_base_update_uops;
...
bool O3_CPU::do_predict_branch(ooo_model_instr& arch_instr)
{
  ...
  if (arch_instr.is_branch) {
    ...
    if (arch_instr.branch_type == BRANCH_CONDITIONAL && arch_instr.branch_taken != arch_instr.branch_prediction) {
      // This is a direction prediction miss
      sim_stats.direction_prediction_miss++;
    }
    if (arch_instr.branch_taken && predicted_branch_target != arch_instr.branch_target) {
      // This is a target prediction miss

      if (predicted_branch_target == 0 && !always_taken) {
        // This is a BTB miss (branch not in BTB table)
     
        if (arch_instr.branch_type == BRANCH_RETURN) {
          // Return BTB miss
          sim_stats.target_prediction_miss_return_not_in_btb++;
        }
        else if (arch_instr.branch_type == BRANCH_INDIRECT ||arch_instr.branch_type == BRANCH_INDIRECT_CALL) {
          // Indirect BTB miss
          sim_stats.target_prediction_miss_indirect_not_in_btb++;
        }
        else {
          // Generic BTB miss
          sim_stats.target_prediction_miss_general_not_in_btb++;
        }
      }
      else {
        if (arch_instr.branch_type == BRANCH_RETURN) {
          // RAS empy or incorrect target from RAS
          sim_stats.target_prediction_miss_return_wrong_target++;
        }
        else if (arch_instr.branch_type == BRANCH_INDIRECT ||arch_instr.branch_type == BRANCH_INDIRECT_CALL) {
          // Incorrect target from indirect
          sim_stats.target_prediction_miss_indirect_wrong_target++;
        }
        else {
          // Incorrect target from BTB table
          sim_stats.target_prediction_miss_general_wrong_target++;
        }
      }
    }
...
long O3_CPU::retire_rob()
{
  ...
  int base_update_count = 0;
  std::for_each(retire_begin, retire_end, [&base_update_count](const auto& x) { if (x.ip % 4 != 0) {base_update_count ++;} });
  num_base_update_uops += base_update_count;
